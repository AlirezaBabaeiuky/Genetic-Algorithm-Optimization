...
CostFunction=@(x,y,z) Sphere(x);     % Cost Function
...
VarMin1 = 0.0;         % Lower Bound of Variables
VarMax1 = 1e3;         % Upper Bound of Variables
VarMin2 = 0.0;         % Lower Bound of Variables
VarMax2 = 1e3;         % Upper Bound of Variables
%VarMin3=0.5*0.5e-3;         % Lower Bound of Variables
%VarMax3=1.5*0.5e-3;         % Upper Bound of Variables
...
for i=1:nPop
    ...
    % Initialize Position
    pop(i).Position(1,1)=(rand(1)*(VarMax1- VarMin1))+VarMin1;
...
for it=1:MaxIt
    ...
    % Sort Population
    Costs=[pop.Cost];
    [Costs, SortOrder]=sort(Costs); % in maximization problem, one needs to mention 'descend'
    % otherwise software by default takes minimization procedure 
    pop=pop(SortOrder);
...
figure;
plot(nfe , BestCost , 'LineWidth' , 3);
xlabel('NFE' , 'FontSize' , 18);
ylabel('Cost' , 'FontSize' , 18);
legend({'Genetic Optimization Algorithm'} , 'FontSize' , 14)
